...
function Dictionary() {
  ...
  const saveTerm = () => { if (term.trim()) { setSavedTerm(term.trim()); setTerm(''); setTeaching(false) } }
  return <div className="dictionary-page page-enter"><div className="utility-heading"><div><h1>dictionary</h1><span className="heading-mark" /></div><button className="import-button">↧ &nbsp; import</button></div><section className="dictionary-proof"><h2>words kivi never misspells.</h2><div className="dictionary-example"><div><span>you say</span><p>“aditya shatriya”</p></div><b>→</b><div><span>kivi writes</span><p>Aditya Kshatriya</p></div></div></section>{teaching ? <div className="teach-form"><input autoFocus value={term} onChange={(event) => setTerm(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && saveTerm()} placeholder="type a word Kivi should remember" aria-label="New dictionary term" /><button onClick={saveTerm}>save term</button><button onClick={() => setTeaching(false)}>cancel</button></div> : <button className="teach-row" onClick={() => setTeaching(true)}><span>+</span> teach Kivi a term</button>}{savedTerm ? <div className="saved-term"><span>remembered</span><strong>{savedTerm}</strong><small>Kivi will keep this spelling</small></div> : <p className="utility-empty">no terms yet — teach Kivi a word</p>}</div>
}
