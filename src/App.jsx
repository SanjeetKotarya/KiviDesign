...
function ImprintsHome({ onOpen, onFirstRun }) { return <div className="imprints-page page-enter"><div className="page-heading"><div><p className="overline">YOUR SPACE · ACCUMULATED, NOT CONFIGURED</p><h1>Where Kivi knows you</h1><p className="subtitle">Each Imprint is a small history of corrections that became instinct.</p></div><span className="settled-mark">✦ 2 contexts, still learning<small>mature voices stay open to the next correction</small></span></div><div className="journal-rule" /><div className="imprint-grid"><ImprintCard imprint={imprints.slack} onClick={() => onOpen(imprints.slack)} /><ImprintCard imprint={imprints.cursor} onClick={() => onOpen(imprints.cursor)} /></div><div className="thin-context"><div className="thin-mark">o</div><div><p className="overline">GMAIL · JUST BEGINNING</p><h3>Not enough history to call it an Imprint yet.</h3><p>3 corrections so far. Kivi is listening, not guessing.</p></div><span>3 corrections</span></div><div className="first-run-path"><p className="no-controls">New to Kivi? Start with a blank slate and let it learn from your first correction.</p><button className="first-run-link" onClick={onFirstRun}>see Kivi before it knows you →</button></div></div> }
...
function Sparkline({ points }) { return <div className="sparkline" aria-label="Correction frequency tapering over time">{points.map((point, index) => <i key={index} style={{ height: `${Math.min(point * 5 + 5, 35)}px` }} />)}</div> }

function ImprintDetail({ imprint, onBack, onMature }) { const isCursor = imprint.id === 'cursor'; return <div className="detail-page page-enter"><button className="back-link" onClick={onBack}>← all Imprints</button><div className="detail-title"><div><p className="overline">{imprint.app.toUpperCase()} · {imprint.context.toUpperCase()}</p><h1>{imprint.app} Imprint</h1><p>{imprint.detail}</p></div><span className={`state large ${imprint.state}`}>{imprint.state}</span></div><section className="detail-proof"><div className="proof-title"><p className="overline">PROVENANCE, NOT SETTINGS</p><h2>What changed because you corrected it</h2></div><div className="detail-pair"><div><span>BEFORE KIVI KNEW</span><p>{imprint.raw}</p><small>heard in {imprint.app} · {isCursor ? '4 days ago' : '3 weeks ago'}</small></div><b>→</b><div className="after"><span>NOW, IN YOUR VOICE</span><p>{imprint.now}</p><small>{imprint.detail}</small></div></div></section><div className="detail-columns"><section><p className="overline">CORRECTION LOG</p>{isCursor ? <><div className="log-entry"><time>MAR 08</time><p>“Please move this...” <b>→</b> “move this...”</p><small>removed padding, kept the instruction</small></div><div className="log-entry"><time>MAR 07</time><p>“use the helper” <b>→</b> “use <code>useSharedQuery</code>”</p><small>learned the exact function name</small></div><div className="log-entry"><time>MAR 05</time><p>“the data thing” <b>→</b> “the response mapper”</p><small>learned the technical term you chose</small></div></> : <><div className="log-entry"><time>MAR 06</time><p>“Could you please...” <b>→</b> “yaar can you...”</p><small>kept it short, kept your opening</small></div><div className="log-entry"><time>MAR 02</time><p>“I will check it” <b>→</b> “dekh leta hoon”</p><small>kept the Hindi phrase you typed</small></div><div className="log-entry"><time>FEB 24</time><p>“Would you mind...” <b>→</b> “can you...”</p><small>removed padding, not meaning</small></div></>}</section><section className="detail-side"><p className="overline">THE PATTERN</p><h3>{imprint.detail}</h3><p>Language resolves itself here from what you correct. No toggle, no preference to maintain.</p><div className="settled-chart"><span>corrections / week</span><Sparkline points={imprint.trend} /></div><button className="course-button" onClick={onMature}>This doesn't sound like me anymore →</button></section></div></div> }
